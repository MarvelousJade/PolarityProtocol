...
namespace PolarityProtocol.Magnetics
{
    ...
    {
        // Preserve meaningful force at the rendered boundary. Without this floor the
        // exponential curve creates a broad near-zero band inside the visible ring.
        private const float EdgeFalloffBase = 0.15f;
            ...
            float falloffExponent = 1.35f)
        {
            ...
            float falloffBase = Mathf.Lerp(1f, EdgeFalloffBase, normalizedDistance);
            float falloff = Mathf.Pow(falloffBase, Mathf.Max(0.1f, falloffExponent));
